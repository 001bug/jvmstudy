...
* 主动使用,分为7种情况
  ...
  5. 初始化一个类的子类
  6. java虚拟机启动时被标明为启动类的类
  7. jdk7 开始提供的动态语言支持: java.lang.invoke.MethodHandle实例的解析结果. REF_getStatic, REF_putStatic,REF_invakeStatic句柄对应的类没有初始化. 则初始化
* 除以上情况,其它使用java类的方式都被看作为是对类的被动使用,不会导致类的初始化
