...
2.  这点也是导致GC进行时必须“Stop The World”的一个重要原因。即使是号称（几乎）不会发生停顿的**CMS**收集器中，**枚举根节点时也是必须要停顿的**。
...
2.  当垃圾回收器发现**没有引用指向一个对象**，即：垃圾回收此对象之前，总会先调用这个对象的finalize()方法。
...
2.  进行筛选，判断此对象是否有必要执行finalize()方法
    1.  如果对象objA没有重写finalize()方法，或者finalize()方法已**经被虚拟机调用过**，则虚拟机视为“没有必要执行”，**objA被判定为不可触及**的。
