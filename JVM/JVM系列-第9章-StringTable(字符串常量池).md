...
1. String的String Pool（字符串常量池）是一个固定大小的Hashtable，默认值大小长度是1009。如果放进String Pool的String非常多，就会造成Hash冲突严重，从而导致链表会很长，而链表长了后直接会造成的影响就是当调用String.intern()方法时性能会大幅下降。
   ...
   String.intern()用于优化内存, 把字符串放入字符串常量池中.并返回池中该字符串的引用. 误解.字符串创建不是默认放在字符串常量池的
...
2.  Java 7中 Oracle的工程师对字符串池的逻辑做了很大的改变，即将字符串常量池的位置调整到Java堆内
    ...
    *   堆空间比较大, 放在堆内可以容纳更多的字符常量, 减少内存溢出的风险.
    *   之前在永久代GC是一件非常耗时的操作(需要跨代操作), 放在堆空间可以和对象统一管理. GC更高效
    *   为JDK8移除永久代做了准备
3.  Java8元空间，字符串常量在堆
...
```java
0 new #2 <java/lang/String> 在堆上创建一个对象
3 dup
4 ldc #3 <ab> 从常量池中索引为3的位置加载一个常量"ab"
...
```java
0 new #2 <java/lang/StringBuilder> 在堆空间中分配一个空间给builder
3 dup
4 invokespecial #3 <java/lang/StringBuilder.<init>>
7 new #4 <java/lang/String> 堆,string
10 dup
11 ldc #5 <a> 常量池中创建一个a
13 invokespecial #6 <java/lang/String.<init>>
16 invokevirtual #7 <java/lang/StringBuilder.append>
19 new #4 <java/lang/String> 堆,string
22 dup
23 ldc #8 <b> 常量池中创建一个b
...
3.  使用一个Hashtable来记录所有的被String对象使用的不重复的char数组。当去重的时候，会查这个Hashtable，来看堆上是否已经存在**一个一模一样的char数组。**
